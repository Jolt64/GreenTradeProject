...
const SEARCH_ITEMS_BT_TITLE = "SEARCH_ITEMS_BT_TITLE";
const SEARCH_ITEMS_BT_CATEGORY = "SEARCH_ITEMS_BT_CATEGORY";
...
export function getCategoryPoints(category) {
  let data = Axios.put("/get-category-points", { category: category }).then(
    res => res.data
  );
  return {
...
export function createNewItem(itemInfo) {
  let data = Axios.post("/create-new-item-post", itemInfo).then(
    res => res.data
  );
  return {
...
export function getUsersItemsList(userID) {
  let data = Axios.get(`/get-user-posted-items/${userID}`).then(
    res => res.data
  );
  return {
...
export function deleteItem(listItemId) {
  let data = Axios.delete(`/delete-posted-item/${listItemId}`).then(
    res => res.data
  );
  return {
...
export function searchItemsByTitle(searchStr) {
  let data = Axios.get(`/search-items-by-title?searchStr=${searchStr}`).then(
    res => res.data
  );
  return {
    type: SEARCH_ITEMS_BT_TITLE,
    payload: data
  };
}

export function searchCategory(category) {
  let data = Axios.get(`/search-items-by-category?category=${category}`).then(
    res => res.data
  );
  return {
    type: SEARCH_ITEMS_BT_CATEGORY,
    payload: data
  };
}
...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case SEARCH_ITEMS_BT_TITLE + "_PENDING":
      return { ...state, loading: true };
    case SEARCH_ITEMS_BT_TITLE + "_FULFILLED":
      return { ...state, listItemsArr: action.payload, loading: false };
    case SEARCH_ITEMS_BT_TITLE + "_REJECTED":
      console.log("rejected");
      return { ...state, loading: false };

    // Search items by category
    case SEARCH_ITEMS_BT_CATEGORY + "_PENDING":
      return { ...state, loading: true };
    case SEARCH_ITEMS_BT_CATEGORY + "_FULFILLED":
      return { ...state, listItemsArr: action.payload, loading: false };
    case SEARCH_ITEMS_BT_CATEGORY + "_REJECTED":
      console.log("rejected");
      return { ...state, loading: false };
